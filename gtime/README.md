...
- **选中即迁移**:选了目标屏后 Dock 立刻移过去。macOS 没有"直接设定 Dock 在哪块屏"
  的 API,只能靠指针压向该屏 Dock 边缘来触发,所以本功能会**隐藏光标**、在该屏
  真正的外沿(边缘外无其它屏处)模拟一次压边,再复位——过程中看不到光标跳动
- 局限:若目标屏的 Dock 边紧贴另一块屏(例如上下叠放、目标屏正下方就是另一块屏),
  那条边不是真正的外沿,macOS 无法在此触发迁移;这种布局请手动放一次或改用侧排/
  把目标屏设为主屏。目标屏未连接时选中不生效(自动跳过)
